    ...
    <div className="px-16 py-10">
      ...
      <div className="py-5 w-1/2">
        {task.description.map((desc, index) => (
          <p key={index} className="text-slate-500 text-sm font-normal text-left mt-3">
            {desc}
          </p>
        ))}
        ...
        <div className="mt-5">
          <h3 className="text-lg font-semibold text-slate-700 mb-2">Documents</h3>
          <div className="grid grid-cols-3 gap-3">
            {task.documents.map((doc, index) => (
              <div key={index} className="bg-white p-3 rounded-lg flex items-center gap-3 border border-gray-200">
                <i className="bi bi-file-earmark-text text-xl text-blue-600"></i>
                <a href={doc.url} className="text-sm text-blue-500 hover:underline">{doc.name}</a>
              </div>
            ))}
          </div>
        </div>
